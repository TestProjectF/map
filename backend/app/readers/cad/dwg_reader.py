...
class DwgReader(BaseCadReader):
    ...
    def inspect(self, path: Path, dataset_id: str, original_file_name: str) -> NormalizedDataset:
        ...
        if not dxf_path:
            signature = path.read_bytes()[:6].decode("ascii", errors="ignore")
            return NormalizedDataset(
                ...
                layers=[],
                warnings=[
                    DatasetWarning(code="DWG_CONVERT_FAILED", message="Không thể chuyển đổi file DWG bằng ODA File Converter.", severity="error")
                ],
                extra={"dwgSignature": signature if signature.startswith("AC") else None, "convertError": self.conversion_error},
            )

        # Sử dụng DxfReader để đọc nội dung
        dxf_reader = DxfReader()
        dataset = dxf_reader.inspect(dxf_path, dataset_id, original_file_name)
        # Ghi đè lại format để UI hiển thị là DWG
    ...
    def create_preview(
        ...
        **kwargs
        ...
        # dataset_id luôn bằng thư mục cha của file DWG nếu nó nằm trong job_dir
